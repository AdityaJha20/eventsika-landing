...
## Corpus Check
- 114 files · ~127,111 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 536 nodes · 1015 edges · 41 communities (24 shown, 14 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 5 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3c5ebe4b`
...
6. `IVendorRepository` - 14 edges
7. `IDeliveryNotifier` - 12 edges
8. `logger` - 12 edges
9. `getSupabaseAdminClient()` - 12 edges
...
Cohesion: 0.13
Nodes (14): Tiered Rate Limiting Architecture, AdminRateLimitResult, checkRateLimitInMemory(), cleanupStaleRecords(), inMemorySingleton, isValidIp(), lastCleanup, MemoryEntry (+6 more)

### Community 1 - "vendor-service.ts"
Cohesion: 0.10
Nodes (22): VENDOR_CATEGORIES, VENDOR_EXPERIENCE_TIERS, DeliveryResult, IDeliveryNotifier, defaultDeliveryNotifier, MailerDeliveryNotifier, maskPhone(), defaultVendorRepository (+14 more)
...
Cohesion: 0.06
Nodes (42): Celebration Intake Pipeline, Repository Pattern & Storage Abstraction, cleanIndianPhone(), formatDisplayDate(), formatDisplayDateTime(), isRecentSubmission(), LeadsWorkspace(), LeadsWorkspaceProps (+34 more)
...
Cohesion: 0.14
Nodes (22): metadata, RFC-4180, buildVendorCsvContent(), cleanIndianPhone(), formatDisplayDate(), formatDisplayDateTime(), sanitizeCsvCell(), sanitizeEmailForMailto() (+14 more)
...
### Community 15 - "login/route.ts"
Cohesion: 0.23
Nodes (13): Fail-Closed Security Policy, DELETE(), GET(), POST(), PUT(), createMockRequest(), getUniqueTestIp(), maskEmail() (+5 more)
...
### Community 24 - "logout/route.ts"
Cohesion: 0.32
Nodes (6): DELETE(), GET(), POST(), PUT(), isAllowedOrigin(), createSupabaseServerClient()

### Community 35 - "leads/route.ts"
Cohesion: 0.27
Nodes (7): dynamic, GET(), dynamic, POST(), LeadService, getOrCreateRequestId(), validateLeadInput()

### Community 36 - "Hero.tsx"
Cohesion: 0.25
...
### Community 38 - "vendor-applications/route.ts"
Cohesion: 0.43
Nodes (6): Reverse-Proxy & Ingress Trust Assumptions, dynamic, POST(), validateVendorInput(), checkRateLimit(), getClientIp()
...
- **Why does `createSupabaseServerClient()` connect `logout/route.ts` to `AdminSidebar.tsx`, `login/route.ts`?**
  _High betweenness centrality (0.168) - this node is a cross-community bridge._
- **Why does `logger` connect `logger.ts` to `rate-limit.ts`, `vendor-service.ts`, `leads/route.ts`, `lead-schema.ts`, `vendor-applications/route.ts`, `VendorsWorkspace.tsx`, `admin-dashboard-service.ts`, `login/route.ts`, `logout/route.ts`?**
  _High betweenness centrality (0.117) - this node is a cross-community bridge._
- **Why does `requireAdminSession()` connect `AdminSidebar.tsx` to `logout/route.ts`?**
